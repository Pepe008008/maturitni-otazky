...
## Operační systém
- **Operační systém** je základní program počítače, který umožňuje běh programů a komunikaci s hardwarem - umožňuje interagovat jiným aplikacím s počítačem
- Rozdělení času procesoru, přidělování operačí paměti, přístup k vnější paměti
- Kernel = interakce hardwaru a softwaru
- Interakce s uživatelem (shell):
...
- Filesystem/systém souborů 
    ...
    - funkce modernách FS:
        ...
        - Šifrování​
        - Symbolické odkazy
- Každý proces má rodiče, který ho pustil, nejčastěji shell nebo jiná aplikace
    - procesy mají exit code, předávají parametry a mohou se řetězit
    - procesy mají PID - identifikátor a vlastníka (owner)
    - stavy: NEW, READY-čeká na cpu, RUNNING, WAITING-čeká na i/o, TERMINATED
    - sledování pomocí
        - ps aux - všechny procesy
        - top
        - kill
        - set - aktuální stav proměnných
        - echo %errorlevel% - vrací exit code procesu
...
## SSH
Otevřít nastavení z admin účtu -> (systém) -> volitelné funkce/části -> klientSSH
- připojení k serveru: ssh -l user@jirovcovka.net seminar.jirovcovka.net
- Unixové prostředí

## Threading
- Každý proces má aspoň jedno vlákno. Celý proces má vlastní paměť
- Vlákna spolu paměť sdílejí, je levné je vytvořit
- scheduler - střídá procesy s frekvencí 1 - 10 ms, tedy vypadají paralelně
- je potřeba hlídat, aby dvě vlákna nezapisovala do stejné paměti -> lock, GIL (global interpretr lock)
- alternativyou asyncio - ideáln pro mnoho i/o souběžně, je bezpečnější
