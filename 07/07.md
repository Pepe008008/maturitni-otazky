...
- **jednočipový PC** = integrovaný obvod obsahující kompletní mikropočítač, používaný např. v automobilových motorech, ovladačích, hračkách atd. (internet of things), Arduina, Raspberry PI
...
- **chipset** - skupina malých obvodů, koordinuje pohyb dat mezi hlavnámi komponenty PC
	...
	![](./chipset.jpg)
- **konektory** - propojují komponenty s základní deskou
...
- části procesoru:
	...
	- **mezipaměť (cache)** - vyrovnávací paměť procesoru, velmi rychlá a drahá, dost ovlivňuje rychlost algoritmů
	- **procesorové hodiny** - synchronizují operace PC
#### Parametry
- frekvence - počet operací sa sekundu - 3-5 GHZ
- velikost cache
- TDP - odpadní teplo, důležité při výběru chlazení
- počet jader - každé jádro dokáže odbavovat daný počet vláken na jednou, na kterých běží procesy. Dnešní standart jsou dvě vlákna na jádro. Běžný počet jader je okolo 10, více se moc nedělá ani na superpočítačích.
...
- **RAM** - (Random access memory) paměť, v níž jsou uloženy běžící programy (včetně operačního systému) a jejich data
	- běžná výbava 16 GB už přestává stačit pro E-sport
![alt text](image-9.png)
- **VRAM** - RAM slouží pro CPU, vedle toho VRAM je pro video a slouží GPU, je rychlejší a lépe dostupná pro GPU
...
- optimalizované pro grafické výpočty, obsahují mnoho velmi primitivních jader, stavěná na násobení matic = grafika
...
- nejnovější modely NVIDIA: RTX 50xx (starší 40xx) - vyšší čísla lepší, (5050-5090), AMD: RX 8000 (8600-8900)
- parametry: frekvence, TDP, konektory, ...
...
### Zdroj
- PSU - Powe supply unit
- Pomocí transformátoru snižuje napětí ze sítě
- Mění střídavý proud na jednosměrný
- Důležité hlídat výkon 400 W - 1000+ W u herních počítačů
