...
### Základní řídicí struktury algoritmů

Každý teoretický algoritmus (a počítačový program) se staví ze tří základních logických bloků: **sekvence, větvení a cyklů**. 

Zde je jejich stručný přehled:

* **Sekvence (Posloupnost):** * Nejzákladnější tok. Příkazy se vykonávají lineárně, přesně jeden po druhém, shora dolů.

* **Větvení (Podmínky):**
    * **If / Else:** Klasický rozhodovací blok. Pokud je podmínka splněna (Pravda), vykoná se první blok kódu. Pokud ne (Nepravda), vykoná se blok `Else`.
    * **Switch (Přepínač):** Používá se, když má jedna proměnná mnoho konkrétních variant a pro každou chceme provést jinou akci (např. rozeznávání dne v týdnu 1-7). Zpřehledňuje kód, aby nebylo nutné psát dlouhé řetězce `If-Else` pod sebe.

* **Cykly (Iterace):**
    
    * **For:** Používá se, když **předem přesně víme, kolikrát** chceme akci opakovat. Definuje se počáteční hodnota, cílová podmínka a krok (např. "počítej od 1 do 10, přidávej po 1").
    * **While (While-Do):** Cyklus s podmínkou na **začátku**. Kód uvnitř se opakuje, dokud je podmínka pravdivá. Pokud podmínka neplatí už při prvním průchodu, cyklus se **neprovede ani jednou**.
    * **Do-While:** Cyklus s podmínkou na **konci**. Zásadní rozdíl oproti *While*: Kód uvnitř cyklu se provede **vždy alespoň jednou**, a teprve poté se zkontroluje podmínka, zda se má cyklus opakovat.

* **Skoky (Nepodmíněné větvení a řízení toku):**
    * **GoTo:** Bezpodmínečný skok na libovolné místo/značku v kódu. V moderním programování se považuje za **špatnou praxi**, protože extrémně znepřehledňuje čtení logiky (vytváří tzv. "špagetový kód"). Dnes je zcela nahrazen strukturovanými cykly a funkcemi.
    * **Break:** Slouží k okamžitému, předčasnému **ukončení celého cyklu** (např. procházíme seznam tisíce položek, najdeme hledanou hodnotu už na třetí pozici – použijeme `break`, abychom neprohledávali zbytek).
    * **Continue:** Přeskočí zbytek kódu v *aktuálním* kroku cyklu a rovnou **začne další opakování** (např. počítáme do deseti, ale číslo 5 chceme vynechat).
